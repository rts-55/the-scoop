...
// my new code added below

function createComment(url, request) {
  const requestComment = request.body && request.body.comment;
  const response = {};

  if (requestComment && database.articles[request.articleId] && database.users[request.username]) {
    const newComment = {
    ...
    database.users[request.username].commentIds.push(newComment.id);
    database.articles[request.articleId].commentIds.push(newComment.id);
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const relatedArticle = database.articles[savedComment.articleId].commentIds;
    relatedArticle.splice(relatedArticle.indexOf(id), 1);

    const relatedUser = database.users[savedComment.username].commentIds;
    relatedUser.splice(relatedUser.indexOf(id), 1);
...
function upvoteComment(url, request) {
  ...
  if (id && user && savedComment && database.users[user] && request.body) {
    savedComment = upvote(savedComment, user);
...
function downvoteComment(url, request) {
  ...
  if (id && user && savedComment && database.users[user] && request.body) {
    savedComment = downvote(savedComment, user);
    response.body = {comment: savedComment};
    response.status = 200;
  } else {
    response.status = 400;
...
// place holders for bonus code: save to database and load from database

const yaml = require('js-yaml');
const fs = require('fs');

// Read YAML file containing database and return js object representing the database
function loadDatabase() {
  const database = yaml.safeLoad(fs.readFileSync('databaseFilePath'));
  return database;
}

// Write current database to YAML file
